...
5. 완성 영상이 연결된 작품은 해당 Grok 쇼츠를 재생하고, 아직 연결되지 않은
   작품은 `쇼츠 준비 중`으로 명확히 표시합니다.
...
로컬 확인 주소는 `http://localhost:3100`입니다. 검증된 변경은 현재 작업
브랜치와 `main`에 푸시하며, Sites 배포는 사용자가 명시적으로 요청할 때만
기존 프로젝트를 갱신합니다.
