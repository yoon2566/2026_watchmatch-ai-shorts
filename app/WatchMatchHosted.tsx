    ...
    <div className="app-shell">
      ...
      <main id="top">
        ...
        {scene === "recommendations" ? <section className="recommendations-section flow-screen" aria-labelledby="recommendations-title">
          ...
          <div className="create-bar"><div><p>{selectedVideo ? "쇼츠 제작 완료" : selected ? "선택한 작품 · 쇼츠 준비 중" : "선택한 작품"}</p><strong>{selected?.title ?? "작품을 하나 골라주세요"}</strong></div><button type="button" className="primary-button" onClick={startProduction} disabled={!selectedVideo}>{selectedVideo ? "제작된 쇼츠 보기" : "쇼츠 준비 중"} <span aria-hidden="true">▶</span></button></div>
        </section> : null}

        {scene === "production" ? <section className="project-section flow-screen production-screen" aria-labelledby="production-title"><section className="pipeline-panel"><div className="pipeline-heading"><div><p className="eyebrow">05 · 영상 준비 중</p><h1 id="production-title" ref={screenTitleRef} tabIndex={-1}>{selected?.title}</h1></div><span className="render-id">READY VIDEO</span></div><div className="active-job"><div className="job-orbit" aria-hidden="true"><span /></div><div><p aria-live="polite">{PIPELINE_STEPS[activePipelineIndex]}</p><span>선택한 작품에 연결된 제작 완료 영상을 준비합니다.</span></div><strong>{progress}%</strong></div><div className="progress-track" role="progressbar" aria-label="쇼츠 준비 진행률" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span style={{width: `${progress}%`}} /></div><ol className="pipeline-steps">{PIPELINE_STEPS.map((label, index) => <li key={label} className={`${index < activePipelineIndex ? "is-complete" : ""} ${index === activePipelineIndex ? "is-active" : ""}`}><span className="step-marker" aria-hidden="true">{index < activePipelineIndex ? "✓" : String(index + 1).padStart(2, "0")}</span><span>{label}</span></li>)}</ol></section><button type="button" className="text-button cancel-button" onClick={() => setScene("recommendations")}>영상 준비 중단</button></section> : null}

        {scene === "result" && selectedVideo ? <section className="project-section flow-screen result-screen" aria-labelledby="result-title"><section className="result-panel"><div className="result-copy"><p className="step-kicker">06 · 영상 보기</p><h1 id="result-title" ref={screenTitleRef} tabIndex={-1}>{selected?.title} 쇼츠를 확인하세요.</h1><p>Watchmode의 실제 검색 결과와 Grok Build CLI로 제작한 전용 MP4를 연결했습니다. 사이트에서 바로 재생하거나 내려받을 수 있습니다.</p><div className="result-facts"><div><span>선택 작품</span><strong>{selected?.title}</strong></div><div><span>검색 OTT</span><strong>{providerLabel}</strong></div><div><span>영상 검증</span><strong>1080×1920 · H.264/AAC</strong></div></div><div className="result-actions"><button type="button" className="primary-button" onClick={showVideoFullscreen}>영상 크게 보기 <span aria-hidden="true">⛶</span></button><a className="secondary-button download-button" href={selectedVideo.src} download>MP4 다운로드 <span aria-hidden="true">↓</span></a><button type="button" className="secondary-button" onClick={startProduction}>다시 보기</button><button type="button" className="text-button" onClick={reset}>처음부터 추천받기</button></div></div><div className="phone-frame"><div className="phone-top" aria-hidden="true"><span /></div><div className="video-shell"><video ref={videoRef} src={selectedVideo.src} controls playsInline preload="metadata" aria-label={`${selected?.title} WatchMatch 쇼츠`}><track kind="captions" src={selectedVideo.captionsSrc} srcLang="ko" label="한국어" default />브라우저에서 영상을 재생할 수 없습니다.</video><span className="video-label">AI 생성 · {selectedVideo.label}</span></div></div></section></section> : null}
      </main>
