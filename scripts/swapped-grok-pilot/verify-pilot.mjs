...
const ffmpeg = "C:\\Users\\User\\Desktop\\채민\\2_작품\\tools\\ffmpeg-portable\\ffmpeg-9.0-essentials_build\\bin\\ffmpeg.exe";
const ffprobe = "C:\\Users\\User\\Desktop\\채민\\2_작품\\tools\\ffmpeg-portable\\ffmpeg-9.0-essentials_build\\bin\\ffprobe.exe";
...
const spoilerTerms = ["결말", "원래 몸으로 돌아온다", "최종 선택의 결과", "죽음", "범인의 정체"];
...
const checks = {
  ...
  titleAndDisclosure:
    manifest.title === "뒤바뀐 친구들의 신비한 모험" &&
    manifest.year === 2026 &&
...
writeFileSync(
  ...
  `${finalSha}  final/swapped_watchmatch_30s.mp4\n`,
  "utf8",
);

const report = {
  project: "WatchMatch Swapped Grok CLI 30-second pilot",
  generatedAt: new Date().toISOString(),
  runDirectory,
  grok: {
  ...
  limitations: [
    ...
    "This pilot uses original generated imagery and only the officially published spoiler-free premise.",
  ],
};
writeFileSync(
  path.join(runDirectory, "swapped_grok_pilot_report.json"),
